...
def run_sweep(env, p_hat, e_vals, sweep_m, sweep_p, discount, q_norm, config, tables, lambda_grid):
 q_tables = {}
...
def main():
 ...
 if mode == "mini":
  ...
  sweep_m = np.linspace(0.0, 1.0, 20)
  sweep_p = np.linspace(0.0, 1.0, 10)
 else:
  sweep_m = np.array([0.0, 0.05, 0.1, 0.15, 0.2, 0.3, 0.5, 1.0])
 ...
 if args.output_root:
  output_dir = os.path.join(args.output_root, args.run_id or f"outputs-idealized-SystemicRisk-{mode}{suffix}-{timestamp}")
 else:
  output_dir = os.path.join(
   os.path.dirname(os.path.abspath(__file__)),
   f"outputs-idealized-SystemicRisk-{mode}{suffix}-{timestamp}",
  )
 ...
 print(f"{'='*60}")
 print(f"Robust MFC Q-Learning — Systemic Risk Idealized Solver (MODULAR)")
 print(f"Config: {mode}")
 ...
 q_tables, results_mean = run_sweep(
  env, p_hat, E_VALS, sweep_m, sweep_p, discount, q_norm, config, tables, lambda_grid
 )

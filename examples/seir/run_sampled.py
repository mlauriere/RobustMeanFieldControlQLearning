...
PROFILE_M_SAMPLED = np.array([0.0, 0.005, 0.01, 0.02, 0.05, 0.1, 0.3, 1.0])
 ...
 for seed in range(num_seeds):
  ...
  for m_idx, m_val in enumerate(sweep_m):
   ...
   Q_star = run_async_qlearning(
    ...
    interleaved_coverage_passes=interleaved_coverage_passes,
    interleaved_coverage_interval=interleaved_coverage_interval,
   )
...
def plot_trajectories(env, tables, Q_m, p_true, T_max, output_dir):
 ...
 fig.tight_layout()
 fig.savefig(os.path.join(output_dir, "plot3_trajectories.png"))
 plt.close(fig)
...
def make_plots(env, tables, q_tables, sweep_m, sweep_p, results, output_dir):
 ...
 ax.set_title(f'{env.name} — Robustness Profiles (Sampled Q-Learning)', fontsize=14)
 ...
 if 0.3 in q_tables and len(q_tables[0.3]) > 0:
  plot_trajectories(env, tables, q_tables[0.3][0], p_true=P_HAT_DEFAULT, T_max=20, output_dir=output_dir)
...
def main():
 ...
 parser.add_argument("--interleaved-coverage-passes", type=int, default=0)
 parser.add_argument("--interleaved-coverage-interval", type=int, default=0)
 ...
 if args.output_root:
  output_dir = os.path.join(args.output_root, args.run_id or f"outputs-async-SEIR-{mode}{suffix}-{timestamp}")
 else:
  output_dir = os.path.join(
   os.path.dirname(os.path.abspath(__file__)),
   f"outputs-async-SEIR-{mode}{suffix}-{timestamp}",
  )
 ...
 print(f"{'='*60}")
 print(f"Robust MFC Q-Learning — SEIR Async Solver (MODULAR)")
 print(f"Config: {mode}")
 ...
 q_tables, results = run_sweep(
  ...
  args.num_seeds, config, tables, args.coverage_passes,
  args.interleaved_coverage_passes,
  args.interleaved_coverage_interval,
  args.w_lr,
  lambda_grid,
  args.seed,
 )
 ...
 np.savez(os.path.join(output_dir, "sweep_results.npz"),
    ...
    interleaved_coverage_passes=args.interleaved_coverage_passes,
    interleaved_coverage_interval=args.interleaved_coverage_interval,

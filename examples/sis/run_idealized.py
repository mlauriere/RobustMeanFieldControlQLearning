...
def main():
 ...
 if args.output_root:
  output_dir = os.path.join(args.output_root, args.run_id or f"outputs-idealized-SIS-{mode}{suffix}-{timestamp}")
 else:
  output_dir = os.path.join(
   os.path.dirname(os.path.abspath(__file__)),
   f"outputs-idealized-SIS-{mode}{suffix}-{timestamp}",
  )
 ...
 print(f"{'='*60}")
 print(f"Robust MFC Q-Learning — SIS Idealized Solver (MODULAR)")
 print(f"Config: {mode}")

...
from sis_env import SISModularEnv
...
def main():
 ...
 parser.add_argument("--cost-dist", type=float, default=0.5)
 parser.add_argument("--c-f", type=float, default=1.8)
 ...
 if args.output_root:
  output_dir = os.path.join(args.output_root, args.run_id or f"outputs-async-SIS-{mode}{suffix}-{timestamp}")
 else:
  output_dir = os.path.join(
   os.path.dirname(os.path.abspath(__file__)),
   f"outputs-async-SIS-{mode}{suffix}-{timestamp}",
  )
 ...
 print(f"{'='*60}")
 print(f"Robust MFC Q-Learning — SIS Async Solver (MODULAR)")
 print(f"Config: {mode}")
 ...
 q_tables, results = run_sweep(
  ...
  args.w_lr, lambda_grid, args.seed,
 )
 ...
 np.savez(os.path.join(output_dir, "sweep_results.npz"),
    ...
    cost_distancing=args.cost_dist, lipschitz_constant=args.c_f,
